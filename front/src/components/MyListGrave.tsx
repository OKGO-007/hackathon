...
const MyListGrave = () => {
    ...
    const { user } = useAuthContext(); // ここでuserを取得
    ...
    const Delete = async (id: string) => {
        if (user) { // userが存在する場合
            ...
            await deleteDoc(doc(db, 'graves', user.uid, 'grave', id));
            closeModal()
        } else {
            console.error("User is not authenticated"); // エラーハンドリング
        }
    };

    const sortedGraves = graves.sort((a: Grave, b: Grave) => {
                ...
                {selectedGrave && (
                    <div>
                        ...
                        <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: '20px' }}>
                            ...
                            <button onClick={() => Delete(selectedGrave.id)} style={{ color: 'red' }}>除去</button>
                        </div>
                    </div>
                )}
            </Modal>
